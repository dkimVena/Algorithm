...
var maxIncreaseKeepingSkyline = function(grid) {
    ...
    for(let i=0; sizeX>i; i++) {
        compVertical.push(Math.max.apply(null, grid[[i]]));
    }
    
    for(let j=0; sizeY>j; j++) {
    ...
    for(let i=0; sizeY>i; i++) {
        for(let j=0; sizeX>j; j++) {
            let tmpNum = Math.min(compVertical[i], compHorizon[j]);
            count += (tmpNum - grid[i][j]) > 0 ? (tmpNum - grid[i][j]) : 0;           
        }
    }
